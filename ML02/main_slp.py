...
def main(dataset_name, data_root, batch_size):
    ...
    hidden_num = math.floor((class_num + in_planes) / 2)
    print("hidden layer node num : ", hidden_num)
	#stack layer
    slp = nn.Sequential(
        nn.Linear(in_planes, class_num)
    )
    
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(slp.parameters(), lr=learning_rate)
